...
prob <- doseresponsebp(alpha, N50, swimmingconcs)
...
DALYhist <- ggplot(DALYslost, aes(x=dalylost)) +geom_histogram(binwidth = 0.005)+theme_linedraw() + labs(title = "DALYs lost to Single Exposure")
probhist <-ggplot(probdf, aes(x=prob)) +geom_histogram(binwidth = 0.00005) +theme_linedraw() + labs(title = "Probability of Infection Single Exposure")
